...
1. **卡牌資訊隨學習等級揭露**：Lv.0 只看到 [???]，打出 3 次後揭露完整資訊
2. **組牌型即學詞性**：要組合就得認識單字的詞性和元素
3. **出牌動畫即學習**：結算時短暫顯示英文→中文→發音
4. **Boss 模糊機制**：Boss 盲注讓卡牌模糊，精通的卡不受影響
5. **商店可選學習強化**：花金幣做聽力/拼字練習升級卡牌
6. **遺忘曲線**：Lv.1-3 有複習間隔，超時降級（籌碼降低），Lv.4 永不退化
...
1. **深化 Build 層級設計**——語法卡（小丑牌等價）、教材卡（星球牌等價）、課程卡（優惠券等價）的具體內容
2. **設計經濟系統**——金幣來源、商店定價、利息機制、重擲成本
3. **設計盲注/Boss 機制**——每大關結構、Boss 特殊限制、跳過獎勵
4. **數值平衡**——目標分數曲線、成長速度、Build 強度測試
5. **設計商店系統**——商品種類、卡包、學習強化的具體互動
6. **設計局外 Meta 系統**——解鎖、牌組、難度、成就
7. **建立新專案或重構現有專案**
