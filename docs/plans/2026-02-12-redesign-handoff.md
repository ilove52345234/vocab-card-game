...
在開始任何後續設計或實作前，接手 Agent 必須先完整閱讀以下三份參考文件：

1. `docs/reference/zhihu-650859964/01.pdf`
2. `docs/reference/zhihu-650859964/02.pdf`
3. `docs/reference/zhihu-650859964/03.pdf`

執行要求：
- 未完成三份文件閱讀，不得開始新的系統設計與程式實作。
- 閱讀後需先在 `docs/SESSION_NOTES.md` 增加「閱讀完成」紀錄（含每份文件至少 1 條關鍵設計結論）再開工。
- 所有新提案必須能回溯對應到上述三份文件的核心觀點（超級概念、極簡驗證+複雜 Build、節奏/隨機/經濟/數值設計）。
...
- ✅ 經濟系統（麻將模型：養牌即 Build、降級不歸零、利息系統）
- ✅ 商店系統（商品區 5 格 + 養牌區 3 格 + 卡包 + 課程卡）
- ✅ 詞庫演進系統（五層池 + 演算法生成牌組 + 滴入式擴展 + 成就加速）
- ✅ 移除防疲勞限制（無每日上限）
- ✅ 盲注/Boss 系統（大盲注=方案二學習聯動限制，Boss=方案三總複習）
- ✅ 數值平衡（Lv.0 ×0.8、答錯扣倍率、3 張起始 Lv.1、前緩後陡曲線、乘算倍率來源）

### 接手 Agent 的待辦任務

1. **設計局外 Meta 系統**——解鎖、牌組、難度、成就
2. **設計局外 Meta 系統**——解鎖、牌組、難度、成就
3. **舊專案轉化分析**——確認哪些程式碼/資料可沿用
4. **建立新專案或重構現有專案**
