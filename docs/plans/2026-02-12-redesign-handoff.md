...
### 2.2 超級概念：單字卡組合（類似撲克牌型/麻將）

**已由用戶確認選擇此方向。**

核心思路：單字有屬性（元素/詞性/長度/字首字尾等），玩家從手牌中選出單字組合成「牌型」得分。

類比 Balatro：
- Balatro 的撲克牌有「花色」和「點數」兩個維度 → 我們的單字有「元素」和「詞性」（或其他維度）
- Balatro 的牌型（對子、順子、同花…）→ 我們的單字組合型（同元素、同詞性、同字首…）
- Balatro 的小丑牌提供被動加成 → 我們需要等價物（可能是字根/語法規則/學習道具）
- 得分 = 基礎分 × 倍率（保持極簡）

### 2.3 學習如何自然發生

關鍵原則：**玩家在玩的過程中不知不覺學會單字，而不是被考試。**

可能的方式：
- 玩家反覆看到單字卡面（英文+中文+圖片），自然記憶
- 組合牌型時需要理解單字屬性，間接學習詞性/分類
- 單字越熟練（玩過越多次），卡牌數值越強 → 內在動機驅動複習
- 拼字/聽力可以作為**可選的加分挑戰**，而非必要條件
...
1. **設計牌型系統**——這是整個遊戲的基礎，決定了什麼維度、什麼組合、什麼分數
2. **設計得分公式和數值框架**——參考 03.pdf 對 Balatro 數值的精密分析
3. **設計 Build 層級**——什麼是我們的「小丑牌」？什麼是「優惠券」？
4. **設計核心循環**——出牌→得分→商店→下一關的完整流程
5. **設計學習融入方式**——熟練度如何影響數值？不打斷心流的前提下如何學習？
6. **建立新專案或重構現有專案**
