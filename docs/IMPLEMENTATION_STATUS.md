...
| 程式架構 | ████░░░░░░ 40% | 核心管理器存在，戰鬥/答題/學習基礎可跑 |
| 系統實作 | ██░░░░░░░░ 20% | 戰鬥循環+答題+SM-2 可跑，協同/地圖/遺物/場所全空 |
...
| 系統 | 設計 | 數據 | 程式碼 | 說明 |
|------|------|------|--------|------|
| **Enums/資料結構** | ✅ | ✅ | ✅ | Enums.cs, WordData.cs, CardData.cs, CombatData.cs |
| **標準值平衡** | ✅ | ✅ | ⚠️ 部分 | cards.json 已校準；程式未讀取 deviation/produces/consumes |
| **維度系統** | ✅ | ✅ | ⚠️ 部分 | cards.json 有 dimension 欄位；程式未使用於協同計算 |
...
| **元素弱點/抗性** | ✅ | ✅ | ❌ | EnemyData 有 weakness/resistance 欄位；傷害計算未納入 |
| **資源媒介協同** | ✅ | ✅ | ❌ | cards.json 有 produces/consumes；程式未實作追蹤 |
| **維度連鎖** | ✅ | — | ❌ | 設計完成；零程式碼 |
| **元素共鳴** | ✅ | — | ❌ | 設計完成；零程式碼 |
| **知識共振** | ✅ | — | ❌ | 設計完成；零程式碼 |
| **普通敵人** | ✅ | ✅ | ⚠️ 部分 | 5 隻普通敵 JSON 齊全；AI 邏輯基礎版 |
| **精英怪** | ✅ | ✅ | ❌ | 3 隻精英 JSON 有數據；特殊機制未實作 |
| **Boss** | ✅ | ✅ | ❌ | 1 隻 Boss JSON 有數據；多階段未實作 |
| **遺物效果** | ✅ | ✅ | ❌ | 18 個遺物 JSON 齊全；效果觸發零程式碼 |
| **地圖生成** | ✅ | — | ❌ | 設計完成（9 條規則）；零程式碼 |
| **休息站** | ✅ | — | ❌ | 設計完成（三選一+進化入口）；零程式碼 |
| **書房** | ✅ | — | ❌ | 設計完成（LP+5 服務）；零程式碼 |
